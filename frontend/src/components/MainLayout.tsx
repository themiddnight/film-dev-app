...
export default function MainLayout() {
  ...
  return (
    <div className="h-full w-full flex flex-col md:flex-row">
      ...
      <main className={`flex-1 min-h-0 ${hideNav ? '' : 'pb-14'} md:pb-0`}>
        <Outlet />
      </main>

      {!hideNav && (
        <nav className="md:hidden h-14 fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-275 border-t border-base-300 bg-base-100 px-2 py-1 grid grid-cols-5 gap-1 z-40">
          {tabs.map((tab) => {
            const Icon = tab.icon
            return (
              <NavLink
