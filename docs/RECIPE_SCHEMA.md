...
>
> ⚠️ ** NOTE** — Root entity `Recipe` (section 2.1) และ `Bath` (section 2.2) ถูก supersede โดย แล้ว
> ดู `DATA_MODEL.md` สำหรับ `Recipe` และ `InventoryItem` แบบใหม่
>
> **Sub-types ที่ยังใช้ได้ใน โดยไม่เปลี่ยน:**
...
Recipe {
 id: uuid
 slug: string // "divided-d23", สำหรับ URL
